...
@[simp] lemma labeledCopyCount_of_isEmpty [IsEmpty W] (G : SimpleGraph V) (H : SimpleGraph W) :
  ...
  rw [labeledCopyCount, ENat.card_eq_one_iff_unique]
...
@[simp] lemma labeledCopyCount_eq_zero : G.labeledCopyCount H = 0 ↔ H.Free G := by
  simp [labeledCopyCount, ENat.card_eq_zero_iff_empty, Free, not_nonempty_iff]
...
@[simp] lemma labeledCopyCount_pos : 0 < G.labeledCopyCount H ↔ H ⊑ G := by
  simp [labeledCopyCount, ENat.card_pos_iff_nonempty, IsContained]
...
lemma copyCount_eq_card_image_copyToSubgraph [Fintype (Copy H G)] [DecidableEq G.Subgraph] :
    copyCount G H = #((Finset.univ : Finset (Copy H G)).image Copy.toSubgraph) := by
  have equiv : {G' : G.Subgraph // Nonempty (H ≃g G'.coe)} ≃
               ↥((Finset.univ : Finset (Copy H G)).image Copy.toSubgraph) :=
  ...
  rw [copyCount, ENat.card_congr equiv, ENat.card_eq_coe_fintype_card, Fintype.card_coe]
...
lemma copyCount_le_labeledCopyCount : G.copyCount H ≤ G.labeledCopyCount H := by
  rw [copyCount, labeledCopyCount]
  apply ENat.card_le_card_of_injective
    (f := fun ⟨G', hG'⟩ ↦ G'.coeCopy.comp (Classical.choice hG').toCopy)
  intro ⟨G₁, _⟩ ⟨G₂, _⟩ h
  simp only [Subtype.mk.injEq]
  simpa [Copy.toSubgraph, Copy.comp, Subgraph.coeCopy, Subgraph.map_comp] using
    congr_arg Copy.toSubgraph h
...
@[simp] lemma copyCount_bot [Finite V] (G : SimpleGraph V) :
  ...
  rw [copyCount, ENat.card_eq_coe_fintype_card]
  norm_cast
...
@[simp] lemma copyCount_of_isEmpty [IsEmpty W] (G : SimpleGraph V) (H : SimpleGraph W) :
    G.copyCount H = 1 := by
  rw [copyCount, ENat.card_eq_one_iff_unique, unique_iff_subsingleton_and_nonempty]
...
lemma le_card_edgeFinset_killCopies_add_copyCount [Finite V] :
    #G.edgeFinset ≤ #(G.killCopies H).edgeFinset + G.copyCount H := by
  classical
  haveI := Fintype.ofFinite V
  obtain rfl | hH := eq_or_ne H ⊥
  · have : (G.killCopies (⊥ : SimpleGraph W)).edgeFinset = G.edgeFinset :=
      Finset.ext fun e => by simp [mem_edgeFinset, killCopies_bot]
    rw [this]; exact le_self_add
  rw [copyCount, ENat.card_eq_coe_fintype_card]
  norm_cast
  rw [← Nat.card_eq_fintype_card]
  suffices h : #G.edgeFinset - Nat.card {G' : G.Subgraph // Nonempty (H ≃g G'.coe)} ≤
               #(G.killCopies H).edgeFinset by omega
  let f (G' : {G' : G.Subgraph // Nonempty (H ≃g G'.coe)}) := (aux hH G'.2).some
  calc
    _ = #G.edgeFinset - card {G' : G.Subgraph // Nonempty (H ≃g G'.coe)} := by
          congr 1; rw [Nat.card_eq_fintype_card]
...
lemma le_card_edgeFinset_killCopies [Finite V] :
    #G.edgeFinset - G.copyCount H ≤ #(G.killCopies H).edgeFinset :=
  tsub_le_iff_right.2 le_card_edgeFinset_killCopies_add_copyCount
