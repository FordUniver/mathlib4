...
theorem bot_strongly_regular : (⊥ : SimpleGraph V).IsSRGWith (⊥ : SimpleGraph V).order 0 ℓ 0 where
  card := rfl
  regular := .bot
  of_adj _ _ h := h.elim
  of_not_adj v w _ := by
...
theorem IsSRGWith.top :
    (⊤ : SimpleGraph V).IsSRGWith (⊤ : SimpleGraph V).order
      ((⊤ : SimpleGraph V).order - 1) ((⊤ : SimpleGraph V).order - 2) μ where
  card := rfl
