...
theorem hasLineDerivAt (h : HasQuadraticLineRemainderWith 𝕜 C f) (x v : E) :
  ...
  intro ε hε
  have hsum_pos : 0 < C * ‖v‖ ^ 2 + 1 := by positivity
  ...
  have ht' : ‖t‖ * (C * ‖v‖ ^ 2 + 1) < ε := (lt_div_iff₀ hsum_pos).mp ht
  have ht'' : C * ‖v‖ ^ 2 * ‖t‖ ≤ ε := by nlinarith [norm_nonneg t]
  calc C * (‖t‖ ^ 2 * ‖v‖ ^ 2)
      = C * ‖v‖ ^ 2 * ‖t‖ * ‖t‖ := by ring
...
theorem isLinearMap_lineDeriv (h : HasQuadraticLineRemainderWith 𝕜 C f) (x : E) :
  ...
  have hg_bigO : g =O[𝓝 0] fun t : 𝕜 => t ^ 2 := by
    ...
    have hab' :
        ‖f b - f a - t • lineDeriv 𝕜 f a (v' - u')‖ ≤
          C * (‖t‖ ^ 2 * ‖v' - u'‖ ^ 2) := by
      convert hab using 1
      rw [show dist a b = ‖b - a‖ by rw [dist_eq_norm, norm_sub_rev], hba,
        norm_smul, mul_pow]
    have haz' :
        ‖f z - f a - c • lineDeriv 𝕜 f a (b - a)‖ ≤
          C * (‖c‖ ^ 2 * (‖t‖ ^ 2 * ‖v' - u'‖ ^ 2)) := by
      convert haz using 1
      rw [show dist a z = ‖z - a‖ by rw [dist_eq_norm, norm_sub_rev], hza, norm_smul,
        hba, norm_smul, mul_pow, mul_pow]
    rw [hba, lineDeriv_smul] at haz'
    have hnorm := norm_sub_le
      (f z - f a - lineDeriv 𝕜 f a (z - a))
      (c • (f b - f a - lineDeriv 𝕜 f a (b - a)))
    rw [norm_smul] at hnorm
    rw [hza, lineDeriv_smul, hba, lineDeriv_smul] at hnorm
    have hg_eq : g t =
        (f z - f a - lineDeriv 𝕜 f a (z - a)) -
          c • (f b - f a - lineDeriv 𝕜 f a (b - a)) := by
      simp only [g, z, a, b]
      rw [hza, lineDeriv_smul]
      module
    rw [hg_eq, hza, lineDeriv_smul, hba, lineDeriv_smul]
    refine hnorm.trans ?_
    rw [norm_pow]
    nlinarith [hab', haz', norm_nonneg t, norm_nonneg c, norm_nonneg (v' - u')]
  have hg_littleO : g =o[𝓝 0] fun t : 𝕜 => t :=
    hg_bigO.trans_isLittleO (isLittleO_pow_id one_lt_two)
