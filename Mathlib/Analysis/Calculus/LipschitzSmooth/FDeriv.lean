...
/-- For a differentiable `K`-smooth `f`, the descent inequality in fderiv form:
`f y ≤ f x + (fderiv ℝ f x) (y - x) + K / 2 · (dist x y)²`. The fderiv restatement of the
canonical `LipschitzSmoothWith` predicate, via `lineDeriv_eq_fderiv` under `Differentiable`. -/
...
`LipschitzSmoothWith.lineDeriv_apply_sub_le`, lifted via `lineDeriv_eq_fderiv`
under `Differentiable`. -/
theorem fderiv_sub_apply_le (h : LipschitzSmoothWith K f)
    (hf : Differentiable ℝ f) (x y : F) :
    (fderiv ℝ f y - fderiv ℝ f x) (y - x) ≤ ↑K * (dist x y) ^ 2 := by
  rw [ContinuousLinearMap.sub_apply, ← (hf y).lineDeriv_eq_fderiv, ← (hf x).lineDeriv_eq_fderiv]
