...
`K`-cocoercivity of the gradient. The converse and the resulting pairwise iff
equivalences between `LipschitzSmoothWith`, `LipschitzWith (fderiv ℝ f)`,
`LipschitzWith (∇ f)`, and `CocoerciveWith` are established in a follow-up that
combines this theorem with the descent lemma.
-/
...
private theorem norm_gradient_sub_sq_le_aux (hc : ConvexOn ℝ Set.univ f)
  ...
  have h_chain : f x + ⟪∇ f x, y - x⟫_ℝ - 1 / K * ⟪∇ f x, g⟫_ℝ ≤
    ...
    calc f x + ⟪∇ f x, y - x⟫_ℝ - 1 / K * ⟪∇ f x, g⟫_ℝ
        = f x + ⟪∇ f x, u - x⟫_ℝ := by
          rw [show u - x = (y - x) - ((1 / (K : ℝ)) • g) from by rw [hu]; module]
      ...
      _ = f y - 1 / K * ⟪∇ f y, g⟫_ℝ + ‖g‖ ^ 2 / (2 * K) := by
          ...
          simp only [inner_neg_right, inner_smul_right, norm_neg, norm_smul,
            Real.norm_eq_abs, abs_of_pos (by positivity : (0 : ℝ) < 1 / K)]
          field_simp; ring
...
theorem cocoerciveWith_of_lipschitzSmoothWith
  ...
  · -- K = 0: descent + FOC pinch `f b = f a + ⟪∇f a, b - a⟫` to equality everywhere,
    ...
    have h_grad_eq : ∀ v : F, ⟪∇ f y, v⟫_ℝ = ⟪∇ f x, v⟫_ℝ := fun v => by
      ...
      linarith
    calc ‖∇ f y - ∇ f x‖ ^ 2
        = ⟪∇ f y - ∇ f x, ∇ f y - ∇ f x⟫_ℝ := (real_inner_self_eq_norm_sq _).symm
      _ = ⟪∇ f y, ∇ f y - ∇ f x⟫_ℝ - ⟪∇ f x, ∇ f y - ∇ f x⟫_ℝ := inner_sub_left _ _ _
      _ = 0 := by linarith [h_grad_eq (∇ f y - ∇ f x)]
      _ ≤ K * ⟪∇ f y - ∇ f x, y - x⟫_ℝ := by rw [hK]; simp
  · have hKp : 0 < (K : ℝ) := lt_of_le_of_ne K.coe_nonneg (Ne.symm hK)
    ...
    have h_sym : ‖∇ f x - ∇ f y‖ ^ 2 = ‖∇ f y - ∇ f x‖ ^ 2 := by rw [← neg_sub, norm_neg]
